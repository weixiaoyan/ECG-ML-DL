...
def CNNnet(inputs,n_class):
    ...
    avg_pool_1 = tf.layers.average_pooling1d(inputs=conv1, pool_size=2, strides=2, \
                                         padding='same')
    conv2 = tf.layers.conv1d(inputs=avg_pool_1, filters=32, kernel_size=3, strides=1,\
                             padding='same', activation = tf.nn.relu)
    avg_pool_2 = tf.layers.average_pooling1d(inputs=conv2, pool_size=2, strides=2,\
                                         padding='same')
    conv3 = tf.layers.conv1d(inputs=avg_pool_1, filters=64, kernel_size=3, strides=1, \
                             padding='same', activation=tf.nn.relu)
    avg_pool_2 = tf.layers.average_pooling1d(inputs=conv2, pool_size=2, strides=2, \
                                             padding='same')
